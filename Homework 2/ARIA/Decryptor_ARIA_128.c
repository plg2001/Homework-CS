...
int decrypt(FILE *in, FILE *out, unsigned char *key, unsigned char *iv) {
    ...
    // Inizializza il contesto per la decifratura con ARIA-128-CBC
    if (EVP_DecryptInit_ex(ctx, EVP_aria_128_cbc(), NULL, key, iv) != 1) {
        handleErrors("Errore nella inizializzazione del contesto");
    }

    unsigned char buffer[BUFFER_SIZE];
    unsigned char decrypted_buffer[BUFFER_SIZE + ARIA_BLOCK_SIZE];
    int len;
    int decrypted_len;
...
int main(){
    ...
    if (iv_bytes_read != ARIA_BLOCK_SIZE) {handleErrors("Errore nella lettura di IV, controlla se generato");}
