...
int decrypt(FILE *in, FILE *out, unsigned char *key, unsigned char *iv) {
    ...
    // Inizializza il contesto per la decifratura con AES-128-CBC
...
int main(){
    ...
    if (iv_bytes_read != AES_BLOCK_SIZE) {handleErrors("Errore nella lettura di IV, controlla se generato");}
